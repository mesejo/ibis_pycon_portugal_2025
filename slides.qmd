...
- 10+ years in data analytics and software engineering
- Passionate about bridging data science and software engineering

:::{.notes}
Hello everyone! I'm Daniel. I'm a Founding Engineer Xorq Labs, where we help organizations modernize their data pipelines. My background is in both data science and software engineering, so I've spent a lot of time translating between pandas analyses and production SQL/ORM code. I'm excited to share how Ibis can eliminate that translation work and serve as a "lingua franca" for data across teams.
:::

## Xorq Labs

- **Xorq Labs** – Startup Data engineering & analytics consulting firm
-  build declarative, reusable ML pipelines across Python and SQL engines like DuckDB, Snowflake, and DataFusion


:::{.notes}
At Xorq Labs, we often see companies where data scientists, data engineers, and developers struggle to collaborate because they're all using different tools. Our mission is to reduce complexity by unifying those workflows. We've been exploring technologies that close the gap between exploratory data analysis and production systems—this is how we got into Ibis.
:::
...
- **Case Study: Only Moka** – a coffee marketplace startup
...
- **Ibis** is an open-source Python **DataFrame library** with a twist
- Provides a **single pandas-like API** for nearly 20 SQL/analytics backends 
...
:::{.notes}
Here's a quick taste of Ibis in action, compared to traditional pandas. On the left, we have a standard pandas workflow: read a CSV, filter out missing prices and countries, then group by country to get price stats. On the right, the Ibis version looks almost identical. We connect to DuckDB and read the CSV into an Ibis table (essentially DuckDB is managing the data). Then we filter and group by country in the same way.

A key difference: when we define summary in Ibis, it hasn't actually run yet. In pandas, the grouping happens immediately in memory. In Ibis, summary is an expression; the actual SQL runs in DuckDB only when we request the data (e.g., converting to pandas or calling execute()). The benefit? Performance and scalability. In our tests with a large coffee dataset (~100 million rows), this Ibis + DuckDB approach was ~14x faster for the aggregation than pandas, and used about 1/4 of the memory. In fact, an official benchmark showed Ibis+DuckDB could be up to 193x faster than pandas for certain analytics workloads while using significantly less memory. The takeaway: you get the convenience of pandas-like code with the power of a database engine.
:::
...
:::{.notes}
To wrap up, I want to emphasize a few key benefits we saw with Ibis at Only Moka:
